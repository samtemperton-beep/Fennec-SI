      ...
      <form onSubmit={save} className="space-y-6">
              ...
              {(['colour', 'emoji', 'upload'] as AvatarTab[]).map(tab => (
                <button key={tab} type="button" onClick={() => setAvatarTab(tab)}
                  style={{
                    ...
                    background: avatarTab === tab ? 'var(--primary)' : 'transparent',
                ...
                {AVATAR_EMOJIS.map(em => (
                  <button key={em} type="button" onClick={() => selectEmoji(em)}
                    style={{
                      ...
                      background: avatarEmoji === em ? 'var(--primary-light)' : 'var(--surface)',
                      outline: avatarEmoji === em ? '2px solid var(--primary)' : 'none',
        ...
        <div className="card">
          <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 16, marginBottom: 16 }}>
            <IconTarget size={16} style={{ display: 'inline', marginRight: 6, color: 'var(--primary)' }} />
            Investment Profile
          </h2>
          ...
          <div style={{ marginBottom: 24 }}>
            <div className="flex items-baseline justify-between mb-2">
              ...
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 13, fontWeight: 700, color: riskLevel <= 3 ? 'var(--green)' : riskLevel <= 6 ? 'var(--amber)' : 'var(--red)' }}>{riskLevel}/10 — {RISK_LABELS[riskLevel]}</span>
            </div>
            <div className="flex items-center gap-3">
              <span style={{ fontSize: 11, color: 'var(--green)', whiteSpace: 'nowrap' }}>Conservative</span>
              <input type="range" min={1} max={10} value={riskLevel} onChange={e => setRiskLevel(Number(e.target.value))} style={{ flex: 1, accentColor: riskLevel <= 3 ? 'var(--green)' : riskLevel <= 6 ? 'var(--amber)' : 'var(--red)' }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {GOAL_PRESETS.map(p => (
                  <button key={p.label} type="button"
                    ...
                    style={{
                      ...
                      background: goalForm.targetPct === p.targetPct && goalForm.months === p.months ? 'var(--primary-light)' : 'var(--surface2)',
                      border: goalForm.targetPct === p.targetPct && goalForm.months === p.months ? '1px solid var(--primary)' : '1px solid var(--border)',
                      color: goalForm.targetPct === p.targetPct && goalForm.months === p.months ? 'var(--primary)' : 'var(--text)',
              ...
              <div className="flex gap-2">
                <button type="button" onClick={saveGoal}
                  style={{ flex: 1, background: 'var(--primary)', color: 'white', padding: '10px', borderRadius: 8, fontFamily: 'Syne, sans-serif', fontWeight: 600, border: 'none', cursor: 'pointer' }}
                >
          ...
          <div className="flex items-start justify-between mb-2">
            ...
            {broker && (
              <a
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'var(--primary)', fontFamily: 'Syne, sans-serif', fontWeight: 600, textDecoration: 'none', flexShrink: 0, marginTop: 2 }}
              >
                  ...
                  {regionBrokers.map(b => (
                    <button
                      ...
                      style={{
                        ...
                        border: broker === b.id ? '1.5px solid var(--primary)' : '1px solid var(--border)',
                        background: broker === b.id ? 'var(--primary-light)' : 'var(--surface2)',
                        color: broker === b.id ? 'var(--primary)' : 'var(--text)',
        ...
        <button type="submit" disabled={saving}
          ...
          style={{ background: 'var(--primary)', color: 'white', fontFamily: 'Syne, sans-serif', fontWeight: 600 }}
        >
